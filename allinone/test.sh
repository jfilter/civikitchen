...
DB_CHECK=$(docker exec ${CONTAINER_NAME} mysql -u civicrm -pcivicrm civicrm -e "SELECT COUNT(*) as count FROM civicrm_contact" 2>/dev/null | tail -n1)
...
for ext in "${EXPECTED_EXTENSIONS[@]}"; do
    if docker exec ${CONTAINER_NAME} /home/buildkit/buildkit/bin/cv api4 Extension.get where='[["key","=",'\"${ext}\"']]' 2>/dev/null | grep -q "\"key\":\"${ext}\""; then
        echo -e "  ${GREEN}✓${NC} ${ext}"
    else
