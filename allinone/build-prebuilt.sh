...
if docker exec "${CONTAINER_NAME}" mysqldump -u root -proot --all-databases --single-transaction > "${DB_DUMP}"; then
    DB_SIZE=$(du -h "${DB_DUMP}" | cut -f1 || true)
...
if docker exec "${CONTAINER_NAME}" tar czf - -C /home/buildkit/buildkit/build site > "${SITE_TARBALL}"; then
    SITE_SIZE=$(du -h "${SITE_TARBALL}" | cut -f1 || true)
    echo -e "${GREEN}✓ Site files exported (${SITE_SIZE})${NC}"
else
