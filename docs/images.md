...
- **cktaint + psalm** — taint analysis only (never a second phpstan): does request input reach a query, shell command, file path or redirect unescaped? Runs against CiviKitchen's CiviCRM source/sink/escape stubs and is **advisory** in CI — see [extension-standards.md](extension-standards.md#taint-analysis-cktaint-advisory-pilot)
