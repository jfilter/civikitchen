...
# felixfbecker/…), and phpstan is a phar precisely so that the static-analysis
# gate every extension depends on cannot be perturbed by another tool's
# dependency tree. Two engines, two roots, no shared resolution.
...
chmod -R a+rX /opt/composer "${CODER_DIR}" "${CIVIKITCHEN_CODER_DIR}" \
    /opt/civikitchen-rector /opt/civikitchen-psalm
