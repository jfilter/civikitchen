...
/**
 * The committed APIv4 catalog must match what the generator produces.
 *
 * Without this, src/Api4Catalog.php silently rots: a core release that adds
 * an entity or a field makes the contract rule reject calls that are fine,
 * which is the one failure mode a checker like this must not have. Skipped
 * when no core checkout is around — CI provides one pinned to CORE_VERSION.
 */
final class Api4CatalogDriftTest extends TestCase
{
    public function testTheCommittedCatalogMatchesTheGenerator(): void
    {
        ...
        $generator = dirname(__DIR__) . '/tools/gen-api4-catalog.php';
        $committed = dirname(__DIR__) . '/src/Api4Catalog.php';
        $fresh = tempnam(sys_get_temp_dir(), 'api4catalog');
        ...
        try {
            ...
            self::assertSame(
                ...
                'src/Api4Catalog.php is stale — regenerate: php tools/gen-api4-catalog.php ' . $coreDir,
            );
        } finally {
            @unlink($fresh);
        }
