...
# Path to seed scripts directory (the profile passes its own via SEEDS_DIR).
SEEDS_DIR="${SEEDS_DIR:-/home/buildkit/scripts/lib/seeds}"
...
seed_extension() {
    ...
    if [[ -f "${seed_script}" ]]; then
        ...
        if is_extension_enabled "${ext_key}"; then
            ...
            # Execute the seed script (best-effort: one flaky seeder must not
            # abort the whole build / skip the rest of the seeds).
            bash "${seed_script}" || echo "  ⚠️  ${ext_key} seed failed (non-fatal)"
        else
